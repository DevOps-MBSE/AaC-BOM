...
from test_utils import run_validator
...
class TestReferencedMaterialValidator(TestCase):
    ...
    def test_valid_model(self):
        ...
        if not result.is_valid():
            raise AssertionError(f"validate_referenced_materials return a validation failure for for a valid material model. isValid = {result.is_valid()} Message = {result.get_messages_as_string()}")
